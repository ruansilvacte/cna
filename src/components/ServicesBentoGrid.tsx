      ...
      >
        {images.map((src, i) => (
          <motion.div key={i} variants={item} className="rounded-2xl overflow-hidden aspect-square">
            <img loading="lazy" decoding="async" src={src} alt="" className="w-full h-full object-cover transition-transform duration-500 hover:scale-105" />
          </motion.div>
        ))}
        <motion.div variants={item} className="rounded-2xl overflow-hidden aspect-square md:hidden">
          <img loading="lazy" decoding="async" src={mobileExtraImage} alt="" className="w-full h-full object-cover transition-transform duration-500 hover:scale-105" />
        </motion.div>
      </motion.div>
    </section>
  );
