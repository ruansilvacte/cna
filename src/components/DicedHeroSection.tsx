        ...
        >
          ...
          >
            {mainText.split(' ').map((word, i, arr) => (
              i >= arr.length - 2 ? (
                <span key={i} className="bg-clip-text text-transparent" style={{ backgroundImage: 'linear-gradient(135deg, hsl(var(--brand-pink)), hsl(var(--brand-pink-soft)), hsl(210 80% 55%))' }}>
                  {word}{i < arr.length - 1 ? ' ' : ''}
                </span>
          ...
          <motion.button
            ...
            style={{ background: 'linear-gradient(135deg, hsl(30 15% 30%), hsl(210 85% 38%), hsl(var(--brand-pink-soft)))', color: 'white', fontFamily: 'var(--font-body)' }}
            variants={fadeUp}
