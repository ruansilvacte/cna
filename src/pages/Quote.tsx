...
const trustBadges = [
  ...
  { icon: MapPin, text: "Serving North Carolina & Massachusetts" },
];
...
export default function Quote() {
  ...
  return (
    <>
      <SeoHead
        slug="quote"
        fallbackTitle="Get a Free Quote | WeHome Cleaning"
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
                ...
                <motion.div className="flex flex-col justify-center"
                  ...
                  <h2 className="text-2xl md:text-3xl font-bold mb-4"
                    ...
                    <span className="bg-clip-text text-transparent"
                      style={{ backgroundImage: "linear-gradient(135deg, hsl(var(--brand-pink)), hsl(var(--brand-pink-soft)))" }}>
                      WeHome Cleaning?
                    </span>
                  </h2>
                  <p className="text-muted-foreground leading-relaxed mb-8">
                    Professional cleaning services across North Carolina & Massachusetts. Reliable, fast, and committed to exceeding your expectations.
                  </p>
                  <div className="space-y-5">
                    {[
                      { icon: ShieldCheck, title: "Trusted & Insured", desc: "Fully insured with consistent 5-star ratings across the Carolinas." },
                      ...
                      <div key={item.title} className="flex gap-4">
                        <div className="shrink-0 w-10 h-10 rounded-xl flex items-center justify-center"
                          style={{ background: "linear-gradient(135deg, hsl(var(--brand-blush)), hsl(var(--brand-pink-soft)))" }}>
                          <item.icon className="w-5 h-5" style={{ color: "hsl(var(--brand-pink))" }} />
                        </div>
                        <div>
                ...
                <div>
                  {status === "success" ? (
                    ...
                    >
                      <div className="mb-6">
                        <h3 className="text-3xl md:text-4xl font-bold text-primary" style={{ fontFamily: "var(--font-heading)" }}>
                          Get a free quote
                        </h3>
                        <p className="text-sm text-foreground mt-3 leading-relaxed">
                          We'll be in touch in less than 10 minutes.
                          <br />
                          <span className="text-muted-foreground">Office hours: Mon–Fri 9:00 AM, 5:00 PM</span>
                        </p>
                      </div>

                      <div className="space-y-3">
                        <input type="text" placeholder="Full name" value={fields.name} onChange={set("name")} className={quoteInputCls(!!errors.name)} />
                        {errors.name && <p className="text-xs text-destructive -mt-2 px-1">{errors.name}</p>}

                        <input type="email" placeholder="Your e-mail" value={fields.email} onChange={set("email")} className={quoteInputCls(!!errors.email)} />
                        {errors.email && <p className="text-xs text-destructive -mt-2 px-1">{errors.email}</p>}

                        <input type="tel" placeholder="(555) 000-0000" value={fields.phone} onChange={set("phone")} className={quoteInputCls(!!errors.phone)} />
                        {errors.phone && <p className="text-xs text-destructive -mt-2 px-1">{errors.phone}</p>}

                        <input type="text" placeholder="ZIP code" value={fields.zip_code} onChange={set("zip_code")} className={quoteInputCls(!!errors.zip_code)} />
                        {errors.zip_code && <p className="text-xs text-destructive -mt-2 px-1">{errors.zip_code}</p>}

                        <input type="text" placeholder="Square footage" value={fields.square_footage} onChange={set("square_footage")} className={quoteInputCls(!!errors.square_footage)} />

                        <select
                          value={fields.notes}
                          onChange={(e) => setFields((f) => ({ ...f, notes: e.target.value }))}
                          className={quoteInputCls(false) + " appearance-none bg-no-repeat pr-10"}
                          style={{ backgroundImage: "url(\"data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='12' height='8' viewBox='0 0 12 8'><path fill='%23666' d='M6 8L0 0h12z'/></svg>\")", backgroundPosition: "right 1rem center" }}
                        >
                      ...
                      <button onClick={goToStep2}
                        className="mt-6 px-10 py-4 rounded-full text-sm font-bold tracking-wider text-white uppercase transition-all hover:scale-[1.02] hover:shadow-lg inline-flex items-center justify-center gap-2"
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-10 max-w-3xl mx-auto">
                      {[
                        ...
                        { icon: MapPin, title: "Carolinas Coverage", desc: "100+ ZIP codes served" },
                      ].map((item) => (
                        <div key={item.title}
                          ...
                          <div className="w-9 h-9 rounded-xl flex items-center justify-center mb-2"
                            style={{ background: "linear-gradient(135deg, hsl(var(--brand-blush)), hsl(var(--brand-pink-soft)))" }}>
                            <item.icon className="w-4 h-4" style={{ color: "hsl(var(--brand-pink))" }} />
                          </div>
                    ...
                    <div className="max-w-3xl mx-auto">
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3 mb-8">
                          ...
                          <div className="flex-1 h-0.5 rounded-full" style={{ background: "hsl(var(--brand-pink))" }} />
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                          ...
                          <div>
                            <div className="flex items-center gap-2 mb-3">
                              <CalendarDays className="w-4 h-4" style={{ color: "hsl(var(--brand-pink))" }} />
                          ...
                          <div>
                            <div className="flex items-center gap-2 mb-3">
                              <Clock className="w-4 h-4" style={{ color: "hsl(var(--brand-pink))" }} />
                        ...
                        {selectedDate && selectedTime && (
                          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                            ...
                            style={{ borderColor: "hsl(var(--brand-pink) / 0.2)", background: "hsl(var(--brand-pink) / 0.04)" }}>
                            <CalendarDays className="w-5 h-5 shrink-0" style={{ color: "hsl(var(--brand-pink))" }} />
...
function quoteInputCls(hasError: boolean) {
  return [
    "w-full px-5 py-4 rounded-xl text-base border bg-secondary/40 text-foreground placeholder:text-muted-foreground",
